...
def _label_for_other_ids(ids: list[str], tag_name_by_id: dict[str, str]) -> str:
    if not ids:
...
def aggregate_month_tag_slices(
    ...
    *,
    ...
    for r in rows:
        merged = _merged_ids(r)
        if filter_tag_id not in merged:
            continue
        others = tuple(sorted(merged - {filter_tag_id}))
        s, c = buckets[others]
        buckets[others] = (s + r.effective_amount, c + 1)
    ...
    for key, (s, c) in top:
        ...
        slice_rows.append(
            MonthTagSliceRow(
                other_tag_ids=ids,
                label=_label_for_other_ids(ids, tag_name_by_id),
                sum_effective=s,
                payment_count=c,
                fraction=frac,
            )
