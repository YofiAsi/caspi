...
from caspi.domain.value_objects.ids import CategoryId, ImportId, PaymentId, RuleId
from caspi.domain.value_objects.money import Money


def _make_payment(merchant: str | None) -> Payment:
    return Payment(
...
def test_no_merchant_returns_no_result():
    service = CategorizationService()
    payment = _make_payment(merchant=None)
...
def test_no_matching_rule_returns_no_result():
    service = CategorizationService()
    payment = _make_payment(merchant="Super Pharm")
