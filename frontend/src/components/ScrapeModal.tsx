      ...
      while (true) {
        ...
        for (const part of parts) {
          ...
          if (event.type === 'start') {
            ...
            setCooldown(null)
            setProgress({ current: event.current, total: event.total, month: event.month })
            ...
            setResults((prev) => [...prev, { month: event.month, error: event.error }])
                ...
                <div className="max-h-40 overflow-y-auto flex flex-col gap-1 text-xs">
                  {results.map((r) => (
                    ...
                    >
                      <span className="font-mono">{r.month}</span>
                      {r.error ? (
                        <span className="truncate max-w-[200px]" title={r.error}>failed</span>
                      ) : (
                        <span>{r.payment_count} txns</span>
                      )}
                    </div>
